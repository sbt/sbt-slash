...
object SlashInspect {
  def inspect = Command(InspectCommand, inspectBrief, inspectDetailed)(parser) {
    case (s, (option, sk)) =>
      s.log.info(output(s, option, sk))
      s
  }
  ...
  def parser: State => Parser[(SlashInspect.Mode, ScopedKey[_])] = (s: State) => spacedModeParser(s) flatMap {
  ...
  val spacedKeyParser: State => Parser[ScopedKey[_]] =
    (s: State) => Act.requireSession(s, token(Space) ~> SlashParser.scopedKeyParser(s))

  def output(s: State, option: Mode, sk: Def.ScopedKey[_]): String =
    {
      val extracted = Project.extract(s)
      import extracted.{ showKey => _, _ }
      // This is the change that I made
      implicit val show = SlashDisplay.showRelativeKey(session.current, structure.allProjects.size > 1, None)
      option match {
        case Details(actual) =>
