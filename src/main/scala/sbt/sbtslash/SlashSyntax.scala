...
trait SlashSyntax {
  ...
  implicit def sbtSlashSyntaxRichProject(p: Project): RichReference =
  ...
  implicit def sbtSlashSyntaxRichScopeAxis(a: ScopeAxis[Reference]): RichScopeAxis =
    new RichScopeAxis(a)

  // This is for sbt 0.13 compat. Remove for sbt 1.0.
  implicit def sbtSlashSyntaxRichFileSetting(s: SettingKey[File]): RichFileSetting =
    new RichFileSetting(s)

  // This is for sbt 0.13 compat. Remove for sbt 1.0.
  implicit def sbtSlashSyntaxRichFilesSetting(s: SettingKey[Seq[File]]): RichFilesSetting =
